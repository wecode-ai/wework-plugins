...
`dws auth login --recommend --format table`，由本机浏览器完成 OAuth 授权，并在
登录后等待服务端推荐权限的确认结果。这里的 table 模式是授权包装器的特例，
用于让 DWS 持续轮询第二段浏览器授权；不能改成会立即返回
`PAT_BATCH_AUTH_PENDING` 的 JSON 模式。只有浏览器授权、
