...
def state_root():
    path = Path.home() / ".wegent-executor/plugin-auth/lark"
...
def environment(source=True):
    ...
    if set(settings) - {"LARKSUITE_CLI_CONFIG_DIR", "XDG_DATA_HOME"}:
    ...
    if source:
        result["WEGENT_LARK_STATE_DIR"] = str(state_root())
    else:
        result.pop("LARKSUITE_CLI_CONFIG_DIR", None)
        result.pop("XDG_DATA_HOME", None)
    return result
...
def companion():
    ...
    blob, metadata = folder / "lark-account-auth.xz", folder / "lark-account-auth.json"
...
def invoke(arguments, *, capture=False, timeout=240):
    source = not (
        len(arguments) > 1
        and arguments[0] in ("user", "bot")
        and arguments[1] in ("run", "refresh", "revoke")
    )
...
def receipt(account):
    name = "account-" + hashlib.sha256(account.encode()).hexdigest() + ".json"
    return state_root() / name


def context():
    result = invoke(["context"], capture=True)
    if result.returncode or len(result.stdout) > 16384:
        return {}
    return json.loads(result.stdout)


def require_idle_source():
    command = (
        ["tasklist", "/FI", "IMAGENAME eq lark-cli.exe", "/FO", "CSV", "/NH"]
        if os.name == "nt"
        else ["pgrep", "-x", "lark-cli"]
    )
    ...
    if os.name == "nt":
        ...
        busy = any(
            row and row[0].lower() == "lark-cli.exe"
            for row in csv.reader(result.stdout.decode(errors="replace").splitlines())
        )
        failed = result.returncode != 0
    else:
        busy, failed = result.returncode == 0, result.returncode not in (0, 1)
    if busy or failed:
        raise AuthError("plugin_auth_source_busy")
...
def adapter_main(identity, arguments):
    if arguments and arguments[0] in ("export", "detach"):
        with locked():
            require_idle_source()
            return invoke([identity, *arguments]).returncode
    return invoke([identity, *arguments]).returncode
